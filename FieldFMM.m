...
%%
    %max(abs([Vp ;-Vp]*hy))
%     EHm = [Q , Q; P -P]*diag(exp([Vp ;-Vp]*hy))*...
%           [0.5*InvQ , 0.5*InvP; 0.5*InvQ -0.5*InvP]*EHb;
%      G2 = diag([exp(-Vp*hy);ones(size(Vp))])*[0.5*InvQ , 0.5*InvP; 0.5*InvQ -0.5*InvP];
%      G1 = diag([ones(size(Vp));exp(-Vp*hy)])*[0.5*InvQ , 0.5*InvP; 0.5*InvQ -0.5*InvP];
%      EHm = G2\(G1*EHb);
%cond(diag([ones(size(Vp));exp(-Vp*hy)])*[0.5*InvQ , 0.5*InvP; 0.5*InvQ -0.5*InvP])
